...
The LLM is scoped to exactly one job: root-cause investigation.
It can call read-only tools but cannot deploy.
"""
...
class Investigator:
    """
    Agentic investigator using LLM for root cause analysis
    
    The investigator follows a loop:
    hypothesize → call diagnostic tool → observe → refine → conclude
    
    The LLM is read-only - it can investigate but cannot deploy.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-2.0-flash-exp"):
        ...
        if not self.api_key:
            print("Warning: No Gemini API key found. Using rule-based fallback.")
        ...
        self.call_count = 0
        self.max_calls = 20  # Stay within free tier
    ...
    def investigate(self, incident) -> InvestigationResult:
        ...
        try:
            self.call_count += 1
            result = self._call_llm(context, tool_results)
            return result
    ...
    def _run_tools(self, incident) -> Dict[str, Any]:
        ...
        # In production, this would use real data from the incident
        # For demo, we return placeholder results
    ...
    def _call_llm(self, context: Dict[str, Any], tool_results: Dict[str, Any]) -> InvestigationResult:
        """Call LLM for root cause synthesis"""
        if not self.client:
        ...
        try:
            ...
            return self._parse_llm_response(response.text, tool_results)
        except Exception as e:
    ...
    def _build_prompt(self, context: Dict[str, Any], tool_results: Dict[str, Any]) -> str:
        ...
        prompt = f"""You are an expert ML reliability engineer investigating a model performance incident.
...
"""
    ...
    def _parse_llm_response(self, response_text: str, tool_results: Dict[str, Any]) -> InvestigationResult:
    ...
    def _rule_based_fallback(self, incident) -> InvestigationResult:
        """Rule-based fallback when LLM is unavailable"""
        return InvestigationResult(
            diagnosis=f"{incident.drift_type} detected with {incident.severity} severity",
            root_cause="Unable to determine without LLM - manual investigation required",
            recommended_action="Escalate to human for manual investigation",
            confidence=0.3,
            tool_calls={},
            reasoning="Rule-based fallback - LLM unavailable"
        )
    
    def _rule_based_fallback_from_tools(self, context: Dict[str, Any], tool_results: Dict[str, Any]) -> InvestigationResult:
        """Rule-based analysis using tool results"""
        # Simple rule-based logic
        top_drift = tool_results['attribute_drift']['top_drift_features'][0] if tool_results['attribute_drift']['top_drift_features'] else None
        ...
        return InvestigationResult(
            ...
            confidence=confidence,
            tool_calls=tool_results,
            reasoning="Rule-based analysis from tool results"
        )

...
python -c "from eval.scenarios import ScenarioRunner; runner = ScenarioRunner(); runner.run_all_scenarios()"
```
...
```bash
python -c "
from eval.scenarios import ScenarioRunner
from eval.scoreboard import Scoreboard, EvalHarness
...
4. Set the main file path to `aegis/ui/streamlit_app.py`
...
aegis/
├── data_plane/
│   ├── replayer.py        # Stream replayer for testing
│   ├── drift_injector.py  # Drift injection for eval
│   └── serving.py         # FastAPI model server
├── control_plane/
│   ├── controller.py      # Deterministic state machine
│   ├── detectors.py       # Evidently + River detectors
│   ├── risk_gate.py       # Decision matrix
│   └── remediation.py     # Retrain / canary / rollback
├── agent/
│   ├── investigator.py    # Agentic investigation (LLM)
│   └── tools.py           # Read-only investigation tools
├── validation/
│   ├── label_free.py      # NannyML CBPE validation
│   └── invariance_suite.py
├── eval/
│   ├── scenarios.py       # Drift injection scenarios
│   └── scoreboard.py      # Eval results tracking
├── ui/

...
class Controller:
    ...
    def start_investigation(self):
        ...
        if self.investigator:
            ...
            self.current_incident.diagnosis = diagnosis.get("diagnosis")
            self.current_incident.root_cause = diagnosis.get("root_cause")
            self.current_incident.recommended_action = diagnosis.get("recommended_action")
            
            self.transition_to(IncidentState.DIAGNATED, f"Investigation complete: {diagnosis.get('root_cause', 'unknown')}")
    ...
    def start_retraining(self, shadow_hold: bool = False):
        ...
        if self.remediation:
            ...
            if retrain_result.get("success"):
                self.transition_to(IncidentState.VALIDATING, "Retraining complete - starting validation")
    ...
    def start_canary(self):
        ...
        if self.remediation:
            ...
            if canary_result.get("success"):
                self.transition_to(IncidentState.PROMOTED, "Canary successful - promoting challenger")
    ...
    def promote_challenger(self):
        ...
        if self.remediation:
            ...
            if promote_result.get("success"):
                self.transition_to(IncidentState.HEALTHY, "Challenger promoted - incident resolved")
